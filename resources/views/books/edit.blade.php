...
@section('content')
    <form action="{{route('books.update',$book->id)}}" method="POST">
        ...
        <div class="form-group">
            <label for="exampleFormControlInput1">Book Quantity</label>
            <input type="text" name="quantity" id="quantity" value="{{$book->quantity}}" class="form-control @error('quantity') is-invalid @enderror" placeholder="Enter Book Quantity">
            @error('quantity')
            <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
